...
\multirow{8}{*}{\textbf{Protocol Extensions}} & \multirow{2}{*}{\textbf{Data Plane}}    & \textit{SR-MPLS} & \cite{ietf-spring-segment-routing-ldp-interop-13,filsfils-spring-segment-routing-ldp-interop-03,ietf-spring-mpls-anycast-segments-02,psarkar-spring-mpls-anycast-segments-02,filsfils-spring-sr-recursing-info-05,ietf-teas-sr-rsvp-coexistence-rec-04,sitaraman-sr-rsvp-coexistence-rec-02,xu-mpls-sr-over-ip-01,bryant-mpls-unified-ip-sr-03,xu-mpls-unified-source-routing-instruction-04} \\ \cline{3-4} 
                                              ...
                                              &                                         & \textit{BGP-LS}  & \cite{ietf-idr-bgp-ls-segment-routing-ext-08,gredler-idr-bgp-ls-segment-routing-ext-04,ietf-idr-te-lsp-distribution-09,ietf-idr-bgpls-segment-routing-epe-15,previdi-idr-bgpls-segment-routing-epe-03,ietf-idr-bgp-ls-segment-routing-msd-01,tantsura-idr-bgp-ls-segment-routing-msd-05,ietf-idr-te-pm-bgp-10,dawra-idr-bgpls-srv6-ext-03,ketant-idr-bgp-ls-bgp-only-fabric-00} \\ \cline{3-4} 
                                              &                                         & \textit{IS-IS}   & \cite{ietf-isis-segment-routing-extensions,previdi-isis-segment-routing-extensions-05,bashandy-isis-srv6-extensions,previdi-isis-ipv6-prefix-sid-03,hegdeppsenak-isis-sr-flex-algo-02,ietf-lsr-flex-algo-00,ietf-isis-segment-routing-msd-12,tantsura-isis-segment-routing-msd-02,ietf-isis-l2bundles-07,ginsberg-isis-l2bundles-02,rfc7810} \\ \cline{3-4} 
                                              &                                         & \textit{OSPF}    & 
                                              \begin{tabular}[c]{@{}l@{}}\cite{ietf-ospf-segment-routing-extensions,psenak-ospf-segment-routing-extensions-05,ietf-ospf-ospfv3-segment-routing-extensions,psenak-ospf-segment-routing-ospfv3-extension-02}\\ \cite{ietf-ospf-segment-routing-msd-14,tantsura-ospf-segment-routing-msd-01,li-ospf-ospfv3-srv6-extensions,ppsenak-ospf-sr-flex-algo-00,ietf-lsr-flex-algo-00,rfc7471}\end{tabular} \\ \cline{3-4} 
                                              &                                         & \textit{PCEP}    & \cite{ietf-pce-segment-routing,sivabalan-pce-segment-routing-03,sivabalan-pce-binding-label-sid-04} \\ \cline{3-4} 
                                              &                                         & \textit{LISP}    & \cite{rodrigueznatal-lisp-srv6-00} \\ \hline
\end{tabular}
\end{table}

We first propose a Taxonomy to classify each standardization document, and then in the next subsection\extended{s} we report \shortver{an overview of the key standardization activities} \extended{and a detailed listing of all the documents}.
...
\begin{itemize}
    ...
    \item SRv6 Network Programming \cite{id-srv6-network-prog} DRAFT
...
\begin{itemize}
    \item A Scalable and Topology-Aware MPLS Dataplane Monitoring System \cite{id-spring-oam-usecase} WG DOCUMENT (Submitted to IESG for Publication)
    ...
    \item SBFD for SR Policies \cite{id-bfd-sr-policy} DRAFT
\end{itemize}}

\extended{\subsection{Performance Measurement}
\begin{itemize}
    \item Packet Loss and Delay Measurement for MPLS Networks \cite{rfc6374} RFC 6374 \ste{is it a Segment Routing specification?}
    \item UDP Return Path for Packet Loss and Delay Measurement for MPLS Networks \cite{rfc7876} RFC 7876
...
\begin{itemize}
    \item Segment Routing interworking with LDP WG DOCUMENT \cite{ietf-spring-segment-routing-ldp-interop-13} replaces \cite{filsfils-spring-segment-routing-ldp-interop-03}
    \item Anycast Segments in MPLS based Segment Routing DRAFT \cite{ietf-spring-mpls-anycast-segments-02} replaces \cite{psarkar-spring-mpls-anycast-segments-02} 
    \item Segment Routing Recursive Information DRAFT \cite{filsfils-spring-sr-recursing-info-05} (\plv{expired})
    \item Recommendations for RSVP-TE and Segment Routing LSP co-existance WG DOCUMENT \cite{ietf-teas-sr-rsvp-coexistence-rec-04} and \cite{sitaraman-sr-rsvp-coexistence-rec-02}
    \item SR-MPLS over IP DRAFT \cite{xu-mpls-sr-over-ip-01} replaces \cite{bryant-mpls-unified-ip-sr-03} and \cite{xu-mpls-unified-source-routing-instruction-04}
\end{itemize}
\subsubsection{SRv6}
\begin{itemize}
    \item IPv6 Segment Routing Header (SRH) WG DOCUMENT \cite{draft-srh}
%    replaces \cite{previdi-6man-segment-routing-header-08}
    \item Insertion of IPv6 Segment Routing Headers in a Controlled Domain DRAFT \cite{voyer-6man-extension-header-insertion-04}
    \item YANG Data Model for SRv6 DRAFT \cite{raza-spring-srv6-yang-01}
\end{itemize}    
\subsubsection{BGP}
\begin{itemize}
...
\begin{itemize}
    ...
    \item Signaling Maximum SID Depth using Border Gateway Protocol Link-State WG DOCUMENT (\plv{expired}) \cite{ietf-idr-bgp-ls-segment-routing-msd-01} replaces \cite{tantsura-idr-bgp-ls-segment-routing-msd-05}
